...
import nested.nested_interface.MessageListener;
...
public class StringFormat {
    public static void main(String[] args) {
        ...
        int age = 30;
        // 1. printf() : 포맷 형태로 출력
        // 형식 지정자를 문자열에 %기호를 사용하여 삽입할 위치와 방식 지정
        System.out.printf("이름 : %s, 나이 : %d\n", name, age);
        
        // 2. String.format() : 포맷팅 된 문자열로 반환 - 하위버전에서 실무 사용!
        String formattedStr = String.format("이름 : %s, 나이 : %d\n", name, age);
        
        // 3. MessageFormat.format : 복잡한 포매팅이 필요할 때
        String formattedStr2 = MessageFormat.format("이름 {0}, 나이 : {1}", name, age);

        // 4. "문자열".formatted("포맷") : 포뱃된 문자열을 반환 ! 자바 13버전에서 추가됨
        String formatted = "이름 : %s, 나이 : %d".formatted(name, age);
        
        // 문자열 포매팅 예시

        // 숫자 또는 금액 포매팅 (%, 천 단위로 콤마형식,   %.n 부동소수점 자리수)
        double dollorPrice = 123400000.56;
        String fmt1 = String.format("가격 : %,.2f 달러", dollorPrice);
        System.out.println(fmt1);

        // 날짜 시간 포매팅
        // %tY %tm %td = 연 월 일 형식 지정자
        // 1$, 2$, 3$ = 첫번째, 두번째, 세번째 인자를 사용하겠다는 의미, (인자 순서가 맞으면 사용할 필요 없음)
