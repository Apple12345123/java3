...
2. protected
    - 상속 관계에서 부모 클래스가 자식 클래스에 접근 가능할 때 사용
    - 다른 패키지에 있을 경우에도.
3. default (package-private)
    - 접근 제한자를 명시하지 앟는 경우
    - 같은 패키지 내부에서만 사용이 가능하다
4. private
    - 해당 멤버를 선언한 클래스에서만 접근 가능 (나만 쓸 수 있음)
...
- 데이터의 무결성을 유지할 수 있음.(갑자기 속도가 음수로 내려간다는 등 데이터가 일반적이지 않은 것을 막음)
- 캡슐화 원칙을 실현하고, 데이터를 보호할 수 있음.(캡슐화 원칙 : )
...
2. Queue 자료구조
    - 선입 선출(FIFO : Fist In First Out) 방식으로 작동
    - 먼저 들어온 데이터가 먼저 나온다.
    - 식당에서 대기줄을 설때
    - 활용
        - 프린터 출력 대기 큐
...
1. 인스턴스 변수
    ...
    - 'static' 이 붙지 않은 변수
    - 더 이상 참조하는 변수가 없을 경우 GC이 제거 (객체 소면 시 함께 제거됨)
    - 객체의 상태를 저장하는데 사용
2. 클래스 변수
    - 클래스 내부에서 선언
...
- static 키워드의 장점과 주의점
    ...
    - 단점 : 캡슐화를 약화시킬 수 있음. 코드 이해가 어려울 수 있음.

## 상속 Inheritance

- 객체지향의 핵심 개념 중 하나
- 한 클래스가 다른 클래스의 필드와 메서들르 이어받아 사용할 수 있음
- 코드의 재사용성을 높이고, 중복을 줄이고, 구조를 체게적으로 관리할 수 있음.
...
- "is-a" 관계를 모델링한다.
    ...
    - HybroCar(자식 클래스) is a Car(부모 클래스).
- 자바에서는 상속 대상은 하나만 선택할 수 있다.(부모 클래스가 두명이 될 수 없다.)
...
- 상속을 사용할 때 주의할 점
    1. "is-a" 관계가 성립할 때만 사용. 기타 다른 경우 예를 들어 "has-a" 관계 일때는 상속보다 포함(Composition)을 사용하는 것이 더 권장된다.
    ...
    2. 너무 깊은 상속 트리는 피한다.
        ...
        - 코드 복잡성이 증가한다.
    3. 부모 클래스의 변경에 주의한다.
        - Super Class 변경 시 모든 자식 클래스의 영향을 미친다.
        - 코드 의존성이 증가한다.
...
- 인터페이스를 사용하는 이유
    1. 제약 : 추상 클래스는 본문을 가진 실행 가능한 일반 메서드가 끼어들 수 있다. 상속받은 자식 클래스에서 기능을 구현하지 않을 수 있는 가능성이 있다.
       . 해당 문제 원천 차단 가능
...
- OOP 5대 원칙 (SOLID)
    ...
    5. DIP : Dependancy Inversion Principle - 의존성 역전 원칙
        - 고수준 모듈은 저수준 모듈에 의존해서는 안되고 추상화에 의존해야 한다. = 세부사항은 추상화에 의존해야 하고, 추상화는 세부사항에 의존해서는 안된다.
...
- 람다식의 형태
    - '(매개변수들) -> 표현식'
    - '(매개변수들) -> 명령문1; 명령문2
    - 매개변수 : 인력값, 괄호 안에 매개변수를 정의하고, 타입을 생략
...
- 다양한 데이터 소스(컬렉션, 배열, Input/Output, 파일 리소스 등)에서 데이터를 추출하여 연속적으로 처리
  -> 함수형 스타일로 추상화, 간결화하게 표현 가능
- 2가지 유형의 연산이 존재
    1. 중간 연산(Intermediate Operation)
        - 하나의 스트림에서 다른 스트림으로 데이터를 전달
        - 입력값이 스트림, 연산 결과(리턴값)도 스트림.
        - 지연 평가(Lazy Evaluation) : 최종 연산이 될 때까지 실제 연산을 실행하지 않음.
            - 불필요한 계산을 최소화 (작업을 미룸)
...
- 프로세스 : 실행 중인 프로그램, 애플리케이션
    - 자신만의 독립적인 메모리 공간을 가짐. -> 다른 프로세스는 서로 메모리 직접 접근 불가
    - 모든 프로세스는 최소 하나 이상의 스레드를 포함
...
- 멀티 스레딩 환경 메모리 구조
    ...
    - 스택 영역 : 각 스레드는 독립적인 스택을 가짐, 메서드 호출시 지역 변수, 매개변수, 리턴 값 등을 저장하는 곳
...
4. Timed Waiting : 지정된 시간만큼 대기하는 상태 ex) sleep
...
- 멀티 스레딩 환경에서 두 개 이상의 스레드나 프로세스가 서로가 보유한 자원을 서로 요구하면서 무한히 대기하는 상태
  => 더 이상 진행할 수 없기 때문에 멈춤
- 교착상태가 발생하는 네 가지고 조건 (동시에 만족)
    1. 상호 배제(Multual Exclusion) : 한 번에 하나의 스레드만 자원을 사용할 수 있음.
    ...
    4. 순환 대기(Circular) : 각 스레드가 순환적으로 다음 스레드가 요구하는 자원을 보유하는 상태
- 철학자들의 식사 문제
- 교착 상태 문제 해결
    - 4가지 조건 중에 하나만이라도 제거하면 교착상태 풀림.
        - 상호배제 : 자원을 다른 스레드들이 동시에 사용할 수 있게
        - 소유 및 대기 조건 제거 : 한 스레드가 자원을 내려놓으면 됨.
        - 비선점 조건 제거 : 선점할 수 있게 하면 도미. (OS)
        - 순환 대기 조건 제거 : 자원에 순서를 할당하고, 순치적으로 자원 요청
...
2. 성능향상 스트림 : BufferedInputStream, BufferedOutputStream. BufferedReader, BufferedWriter
    - 데이터를 처리할 때 내부적으로는 버퍼를 사용해서 데이터를 임시 저장하여 입출력 효윺을 높임.
...
### IP 주소 (Internet Protocol Address)
        ...
        - 서브넷 마스크
            ...
            - 예시 : 255.255.255.0
                ...
                - 한 개의 네트워크에 255 개의 장치를 가질 수 있다.
                - 첫 숫자(0)과 마시막 숫자(255)는 사용 불가
        - 게이트웨이
            - 서로 다른 네트워크 또는 서브 네트워크 간의 통신을 가능하게 하는 네트워크 장치
            - 일반적으로 라우터(공유기)가 게이트웨이 역할을 수행
        ...
        - DNS (Domain Name System)
            ...
            - 도메인 이름으로 변환해주는 시스템
            - 예시 : www.naver.com
            - 도메인 네임 서버 : 이름 - IP를 매치시키는 역할을 하는 서버
        ...
        - 포트 번호
            ...
            - 분류
                1. 잘 알려진 포트 : 0 ~ 1023 IANA에서 할당
                    ...
                    - FTP (파일 전송) : 21
                    - SSH (보안 접속) : 22
                2. 등록 포트 : 1024 ~ 49151, 특정 서비스에 할당될 수 있음 (사용자 정의)
                    - (사용자 정의), 변경될 수 있음
                    - MySql : 3306
                    - Oracle : 1521
...
## JDBC(Java DAtaBase Connectivity)
    ...
    - API가 데이터페이스에 독립적이어서 다양한 종류의 DBMS에 연결할 수 있는 방법 제공
    - 구성 클래스
...
### JDBC 드라이버 설치
    - JDBC 드라이버르 ㄹ각 제조사 홈페이지에서 검색하여 다운로드 받고
    - MySQL의 경우 Installer에서 Connector/J 를 설치허거나
...
2. Statement 객체
    ...
    - 메서드
      ...
      - executeUpdate(String sql) : DML(INSERT, UPDATE, DELETE) 사용
        - 테이블에 변경된 행의 개수를 반환
      - close() : 객체 자원 해제
    - 객체를 즉시 해제하지 않으면 메모리에 무시할 수 없는 공간이 남게 되기 때문에 꼭 자원 해제
3. ResultSet 객체
